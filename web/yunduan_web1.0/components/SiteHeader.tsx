...
const defaultNavLinks: NavLink[] = [
  { label: '船长的角落', url: '#intro', openInNewTab: false },
  { label: '视频', url: '#videos', openInNewTab: false },
  { label: '文章', url: '#articles', openInNewTab: false },
  { label: '课程', url: '#courses', openInNewTab: false },
  { label: '关于我们', url: '/about', openInNewTab: false },
  { label: '联系合作', url: '/contact', openInNewTab: false },
]
...
export function SiteHeader({
  ...
  const renderLink = (link: NavLink, onClick?: () => void) => {
    const className = "text-base font-bold text-[var(--editor-muted)] hover:text-[var(--editor-ink)] transition-colors duration-150 lg:text-lg"
  ...
  const renderLogo = () => {
    if (theme === 'terminal') {
      return (
        ...
        >
          ...
          <span style={{ color: 'var(--editor-muted)' }}>captain@nook:~$</span>
    ...
    return (
      <Link
        href="/"
        className="text-3xl font-black tracking-tight text-[var(--editor-ink)] hover:text-[var(--editor-accent)] transition-colors duration-200 flex-shrink-0 [font-family:var(--font-display-rounded)]"
      >
        船长的角落
      </Link>
    )
  ...
  return (
    <header className={`site-header ${stickyOnMobile ? 'sticky' : 'sm:sticky'} top-0 z-40 border-b border-[var(--editor-line)] bg-[var(--background)]/95 backdrop-blur-sm`}>
      <div className="site-header-inner mx-auto max-w-7xl px-4 sm:px-6">
        <div className="h-14 flex items-center justify-between gap-4">
          {renderLogo()}

          {/* Desktop nav */}
          <nav className="hidden sm:flex items-center gap-5 text-base flex-shrink-0 lg:gap-7 lg:text-lg">
            {/* Category dropdown */}
            {categories.length > 0 && (
              <div ref={categoryRef} className="relative">
                <button
                  onClick={() => setCategoryOpen(!categoryOpen)}
                  className={`inline-flex items-center gap-1 text-base font-bold transition-colors duration-150 lg:text-lg ${
                    activeCategorySlug
